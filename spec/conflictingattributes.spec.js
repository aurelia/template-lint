...
describe("ConflictingAttributes Rule", () => {
    ...
    it("will reject element where repeat and if is used", (done) => {
        linter.lint(DEFAULT_RULES_VIOLATION_SAMPLE)
            ...
            expect(errMsg).toContain(conflictingattributes_1.ConflictingAttributesRule.TEMPLATE_CONTROLLER_ATTRIBUTES_ERRMSG_PREFIX);
    ...
    it("can be constructed without arguments", (done) => {
        let rule = new conflictingattributes_1.ConflictingAttributesRule();
        expect(rule.conflictingAttributesList).toEqual(conflictingattributes_1.ConflictingAttributesRule.createDefaultConflictingAttributes());
        done();
    });
    ...
    it("allows replacing rules through constructor argument", (done) => {
        ...
        const customConflictingAttributes = new conflictingattributes_1.ConflictingAttributes(["disabled", "onclick"], customErrMsg);
        const linter = new template_lint_1.Linter([
            new conflictingattributes_1.ConflictingAttributesRule([customConflictingAttributes])
        ]);
...
//# sourceMappingURL=conflictingattributes.spec.js.map
